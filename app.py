"""
Free Fire Bot API Server
Deploy on Vercel or Render
Endpoints:
- /access?access={token}&open_id={open_id}&region={region}
- /mafu?uid={uid}&Password={password}&region={region}
- /stopaccess?access={token}
- /stopmafu?uid={uid}&Password={password}
"""
...
# Region-based MajorLogin URLs (exact format as provided)
REGION_URLS = {
    "IND": {  # India
        "login": "https://loginbp.ggpolarbear.com/MajorLogin",
        "name": "India"
    },
    "ME": {  # Middle East
        "login": "https://loginbp.ggpolarbear.com/MajorLogin",
    ...
    "DEFAULT": {
        "login": "https://loginbp.ggblueshark.com/MajorLogin",
        "name": "Default"
    }
}
    ...
    <body>
        ...
        <script>
            fetch('/status').then(r => r.json()).then(d => {
                document.getElementById('botCount').textContent = d.total_bots || 0;
            });
        </script>
    </body>
    </html>
...
async def access_endpoint(
    ...
    open_id: str = Query(None, description="Open ID (optional, will use first 32 chars of token if not provided)"),
    region: str = Query("IND", description="Region code (IND, ME, VN, BD, PK, SG, BR, NA, ID, RU, TH)")
):
    """
    Start bot with access token
    Example: /access?access=TOKEN&open_id=OPENID&region=IND
    """
    # Generate open_id from token if not provided
    if not open_id:
        open_id = access[:32] if len(access) >= 32 else access
    
    # Check if bot already running with this token
...
async def mafu_endpoint(
    ...
    Example: /mafu?uid=4309864974&Password=PASS&region=BD
    """
    # Check if bot already running with this uid
    bot_id = f"mafu_{uid}"
    if bot_id in active_bots and active_bots[bot_id]["bot"].is_running:
        return JSONResponse({
            "success": False,
...
async def stopaccess_endpoint(
    ...
    Example: /stopaccess?access=TOKEN
    """
    bot_id = f"access_{access[:16]}"
...
async def stopmafu_endpoint(
    ...
    Password: str = Query("", description="Password (optional, for verification)")
):
    """
    Stop bot started with guest credentials
    Example: /stopmafu?uid=4309864974&Password=PASS
    """
    bot_id = f"mafu_{uid}"
...
# ============================================================
# VERIFICATION ENDPOINT (for Vercel)
# ============================================================

@app.get("/api/verify")
async def verify_endpoint():
    """Verification endpoint for deployment"""
    return {
        "verified": True,
        "message": "Free Fire Bot API is running",
        "endpoints": ["/access", "/mafu", "/stopaccess", "/stopmafu", "/status", "/regions"]
    }
